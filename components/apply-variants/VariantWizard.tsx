...
export default function VariantWizard() {
  ...
  const {
    ...
    defaultValues: defaultApplyValues("wizard"),
    mode: "onTouched",
  });
    ...
    <form onSubmit={(e) => e.preventDefault()} noValidate>
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div>
            <h3 className="font-display text-2xl font-bold text-navy mb-1">Your Order</h3>
      ...
      {step === 2 && (
        <div className="space-y-6">
          <div>
            <h3 className="font-display text-2xl font-bold text-navy mb-1">Payment</h3>
      ...
      {step === 3 && (
        <div className="space-y-6">
          <div className="rounded-xl border border-gold/30 bg-gold/5 p-4">
            <p className="font-semibold text-navy text-sm">Your order is set. 🎉</p>
            <p className="text-sm text-muted mt-0.5">A few details finish your listing — or have our team collect them for you.</p>
          </div>
      ...
      {/* Navigation */}
      <div className={`flex mt-8 pt-6 border-t border-cream-dark ${step > 1 ? "justify-between" : "justify-end"} items-center gap-3`}>
        {step > 1 && (
          <button type="button" onClick={() => setStep((s) => s - 1)} className="inline-flex items-center gap-2 text-sm font-semibold text-muted hover:text-navy transition-colors">
        ...
        {step < 3 && (
          <button type="button" onClick={goNext} className="inline-flex items-center gap-2 rounded-xl bg-navy px-6 py-3 text-sm font-semibold text-white hover:bg-navy/90 transition-colors">
            {step === 2 ? "Continue to details" : "Continue"} <ChevronRight className="h-4 w-4" />
          </button>
        )}
