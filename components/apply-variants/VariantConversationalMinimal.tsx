...
import { ArrowRight, ArrowLeft, Plus, Trash2, Sparkles, CornerDownLeft } from "lucide-react";
...
export default function VariantConversationalMinimal() {
  ...
  const {
    ...
    defaultValues: defaultApplyValues("conversational-minimal"),
    mode: "onTouched",
  });
  ...
  const screens: Screen[] = [
    ...
    },
    {
      ...
      render: () => (
        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-5 ${inputShell}`}>
      ...
      render: () => (
        <div className="space-y-5 max-w-2xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button
              type="button"
              onClick={() => { setValue("featuredPlacement", false); setScreen((s) => s + 1); }}
              aria-pressed={!watchedFeatured}
          ...
          {watchedFeatured && (
            <div className="flex items-center justify-between gap-4 pt-1">
              <span className="font-display text-xl font-bold text-gold">{formatCurrency(orderTotal)}</span>
              <button type="button" onClick={() => setScreen((s) => s + 1)} className="inline-flex items-center gap-2 text-sm font-semibold text-navy hover:text-gold transition-colors">
  ...
  const HEADINGS: { heading: string; helper?: string }[] = [
    { heading: "What's your business called?", helper: "We'll use this on your listing." },
    { heading: "And your name?", helper: "So we know who to follow up with." },
    { heading: "Where should we send your confirmation?", helper: "We'll email your order details here." },
    { heading: "Best phone number to reach you?", helper: "Only used if we need to confirm something." },
    { heading: "Which cities do you operate in?", helper: "Add every city you want a listing in." },
    { heading: "Which categories fit your business?" },
    { heading: "Want the Top Spot in your city?" },
    { heading: "Lock it in.", helper: "Secure your spot — your listing goes live at the October 2026 launch." },
    { heading: "Your order's in! 🎉 Finish your listing now, or later?" },
    { heading: "A few details finish your listing.", helper: "Or have our team collect them for you." },
  ];

  const currentScreen = screens[screen];
  const { heading, helper } = HEADINGS[screen];
  const isDecision = currentScreen.id === "decision";
  const isFeatured = currentScreen.id === "featured";
  const isDetails = currentScreen.id === "details";
  // Pure single-choice screens drive themselves — no Back, no Next chrome.
  const isChoice = isDecision || isFeatured;
  // A single text input is on screen — show the press-Enter affordance.
  const isSingleInput = currentScreen.fields.length === 1 && currentScreen.id !== "locations";
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} onKeyDown={handleKeyDown} noValidate>
      ...
      {/* Minimal top rail: low-contrast back arrow + tiny counter */}
      <div className="flex items-center justify-between mb-10 h-5">
        {screen > 0 && !isChoice ? (
          <button type="button" onClick={() => setScreen((s) => s - 1)} className="text-muted/60 hover:text-navy transition-colors" aria-label="Back">
            <ArrowLeft className="h-5 w-5" />
          </button>
        ) : (
          <span />
        )}
        <span className="text-xs font-semibold tracking-[0.2em] text-muted/70 tabular-nums">
          {pad(screen + 1)} / {pad(totalScreens)}
        </span>
      </div>

      <div className="min-h-[440px] flex flex-col justify-center">
        <div className="mb-10">
          <h3 className="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-navy leading-[1.1]">{heading}</h3>
          {helper && <p className="text-base text-muted mt-4 max-w-xl">{helper}</p>}
        </div>

        <div>{currentScreen.render()}</div>

        {isSingleInput && (
          <p className="mt-6 inline-flex items-center gap-1.5 text-xs text-muted/70">
            press Enter <CornerDownLeft className="h-3.5 w-3.5" />
          </p>
        )}
      </div>

      {serverError && <div className="mt-8 text-sm text-red-700" role="alert">{serverError}</div>}

      {/* Navigation — only for non pure-choice screens */}
      {!isChoice && (
        <div className="flex mt-10 items-center justify-end gap-3">
          {isDetails ? (
            ...
            <button type="button" onClick={goNext} className="inline-flex items-center gap-2 text-base font-semibold text-navy hover:text-gold transition-colors">
