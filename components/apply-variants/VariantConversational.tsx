...
export default function VariantConversational() {
  ...
  const {
    ...
    defaultValues: defaultApplyValues("conversational"),
    mode: "onTouched",
  });
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} onKeyDown={handleKeyDown} noValidate>
      ...
      {/* Progress bar */}
      <div className="h-1.5 w-full rounded-full bg-cream-dark overflow-hidden mb-2">
        <div className="h-full rounded-full bg-gold transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>
      <p className="text-[11px] font-semibold uppercase tracking-wider text-muted mb-8">Step {screen + 1} of {screens.length}</p>
